...
class ChatHandler:
    def __init__(self):
        logger.debug("Initializing ChatHandler with services")
    ...
    async def stream_openai_response(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            async for chunk in self.openai_service.generate_text_stream(settings, messages):
                full_response += chunk
    ...
    async def handle_image_generation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # 2. Generate image URL via OpenAI service
            image_url = await self.openai_service.generate_image(settings, prompt)
            if not image_url:
                await response_message.edit_text("❌ Не удалось сгенерировать изображение.")
                return
            
            # 3. Download image data
            image_data = await self.openai_service.download_image(image_url)
            if not image_data:
    ...
    async def handle_image_variation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # 3. Create variation URL via OpenAI service
            variation_url = await self.openai_service.create_image_variation(settings, bytes(image_data))
            if not variation_url:
                await response_message.edit_text("❌ Не удалось создать вариацию.")
                return
            
            # 4. Download the new image
            variation_data = await self.openai_service.download_image(variation_url)
